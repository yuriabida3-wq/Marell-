...
<div class="mb-6">
  ...
  <p class="text-sm text-gray-500">Next ADM will be <span class="font-mono font-bold text-navy">{{ $nextAdm }}</span> (auto-assigned)</p>
</div>
...
<div class="bg-white rounded-2xl p-6 md:p-8 shadow max-w-3xl">
  <form method="POST" action="{{ route('principal.students.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
    @csrf
    ...
    <div class="md:col-span-2">
      ...
      <input name="total_fee" type="number" min="0" step="100" value="{{ old('total_fee', 45000) }}" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4 focus:border-gold focus:outline-none">
    </div>
    ...
    <div class="md:col-span-2 flex flex-col sm:flex-row gap-3">
      ...
      <a href="{{ route('principal.students.index') }}" class="flex-1 min-h-[48px] rounded-xl bg-gray-100 text-gray-700 font-semibold text-center leading-[48px] hover:bg-gray-200 transition">Cancel</a>
    </div>
  </form>
</div>
