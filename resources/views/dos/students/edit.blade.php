...
<div class="mb-6">
  <a href="{{ route('dos.students.show', $student) }}" class="text-xs text-gray-500">← Back to profile</a>
...
<div class="bg-white rounded-2xl p-6 md:p-8 shadow max-w-3xl">
  <form method="POST" action="{{ route('dos.students.update', $student) }}" class="grid grid-cols-1 md:grid-cols-2 gap-4">
    @csrf @method('PUT')

    <div class="md:col-span-2">
      <label class="block text-xs font-semibold text-navy mb-2 tracking-widest">STUDENT NAME *</label>
      <input name="name" value="{{ old('name', $student->name) }}" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    </div>
    ...
    <div class="md:col-span-2">
      ...
      <input name="parent_name" value="{{ old('parent_name', $student->parent_name) }}" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    </div>

    <div>
      <label class="block text-xs font-semibold text-navy mb-2 tracking-widest">PARENT PHONE *</label>
      <input name="parent_phone" value="{{ old('parent_phone', $student->parent_phone) }}" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    </div>

    <div>
      <label class="block text-xs font-semibold text-navy mb-2 tracking-widest">PARENT EMAIL</label>
      <input name="parent_email" type="email" value="{{ old('parent_email', $student->parent_email) }}" class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    </div>

    <div>
      <label class="block text-xs font-semibold text-navy mb-2 tracking-widest">TOTAL FEE (KES) *</label>
      <input name="total_fee" type="number" min="0" step="100" value="{{ old('total_fee', $student->total_fee) }}" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    </div>

    <div>
      <label class="block text-xs font-semibold text-navy mb-2 tracking-widest">STATUS *</label>
      <select name="status" required class="w-full min-h-[48px] rounded-xl border-2 border-gray-200 px-4">
    ...
    <div class="md:col-span-2 flex gap-3">
      <button class="flex-1 min-h-[48px] rounded-xl bg-gold text-navy font-bold">💾 Save Changes</button>
      <a href="{{ route('dos.students.show', $student) }}" class="flex-1 min-h-[48px] rounded-xl bg-gray-100 text-gray-700 font-semibold text-center leading-[48px]">Cancel</a>
    </div>
  </form>
</div>
@endsection
